...
type PlaylistPickerProps = {
  isOpen: boolean
  onClose: () => void
  onSelect: (playlistId: string, playlistName: string, refillThreshold: number) => void
}
...
export function PlaylistPicker({ isOpen, onClose, onSelect }: PlaylistPickerProps) {
  ...
  const [refillInput, setRefillInput] = useState('0')
  ...
  const handleConfirm = () => {
    ...
    const threshold = Math.max(0, parseInt(refillInput, 10) || 0)
    onSelect(selectedPlaylist.id, selectedPlaylist.name, threshold)
    onClose()
  }
